import httpx
import asyncio
import time
import os  # 新增：用于读取环境变量


class RocomTargetBot:
	def __init__(self):
		# 🔒 隐私优化：从环境变量读取配置
		self.api_key = os.getenv("MY_API_KEY")
		self.webhook_url = os.getenv("MY_WEBHOOK")
		self.api_base_url = "https://wegame.shallow.ink"
		self.target_keywords = ["棱镜", "棱彩", "祝福", "炫彩", "国王", "血脉秘药"]

	async def get_filtered_products(self):
		"""获取商品并筛选包含关键词的在售物品"""
		if not self.api_key:
			print("错误: 未找到 API Key")
			return None

		path = "/api/v1/games/rocom/merchant/info"
		now_ms = time.time() * 1000
		headers = {"X-API-Key": self.api_key}

		async with httpx.AsyncClient(timeout=15.0) as client:
			try:
				resp = await client.get(f"{self.api_base_url}{path}", headers=headers)
				data = resp.json()
				if data.get("code") != 0:
					return None

				res_data = data.get("data", {})
				activities = res_data.get("merchantActivities") or res_data.get("merchant_activities") or []
				if not activities:
					return None

				props = activities[0].get("get_props", [])
				hit_items = []
				for item in props:
					name = item.get("name", "")
					start_time = item.get("start_time")
					end_time = item.get("end_time")

					if start_time and end_time:
						if not (int(start_time) <= now_ms < int(end_time)):
							continue

					if any(kw in name for kw in self.target_keywords):
						st_str = time.strftime("%H:%M",
						                       time.localtime(int(start_time) / 1000)) if start_time else "08:00"
						et_str = time.strftime("%H:%M", time.localtime(int(end_time) / 1000)) if end_time else "23:59"
						hit_items.append(f"· {name} ({st_str}-{et_str})")
				return hit_items
			except Exception as e:
				print(f"数据获取异常: {e}")
				return None

	async def send_webhook(self, hit_list):
		if not self.webhook_url:
			print("错误: 未找到 Webhook URL")
			return

		title = "📢 【洛克王国】物资刷新提醒！\n"
		body = "\n".join(hit_list)
		footer = f"\n\n⏰ 检测时间：{time.strftime('%Y-%m-%d %H:%M:%S')}"

		payload = {
			"msgtype": "text",
			"text": {
				"content": f"{title}---------------------------\n{body}{footer}",
				"mentioned_list": ["@all"]
			}
		}

		async with httpx.AsyncClient() as client:
			await client.post(self.webhook_url, json=payload)

	async def run(self):
		hit_list = await self.get_filtered_products()
		if hit_list:
			await self.send_webhook(hit_list)
		else:
			print("未发现目标物品。")


if __name__ == "__main__":
	bot = RocomTargetBot()
	asyncio.run(bot.run())